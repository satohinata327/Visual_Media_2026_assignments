"""
POPEの最初の回答tokenでclean側がNoを
高い確信度で支持する場合だけ、VCD補正を行わないGateを追加
"""
...
def sample(
    ...
    **model_kwargs,
    ...
    # POPEでは最初の生成tokenがYes/Noに対応するため、最初のtokenだけ
    # Gateを適用できるよう、生成開始時の入力長を保存
    ...
    while True:
        ...
        if use_cd:
            ...
            # clean側がNoを高確信度で支持する場合、VCDによるYes方向への
            # 過剰な反転を防ぐため、最初の回答tokenだけclean logitsを使用する
            # thresholdがNoneなら公式VCDと同じ処理
            gate_threshold = getattr(self, "precision_gate_threshold", None)
            is_first_generated_token = input_ids.shape[-1] == initial_input_length

            if gate_threshold is not None and is_first_generated_token:
                yes_token_id = self.precision_gate_yes_token_id
                no_token_id = self.precision_gate_no_token_id

                logit_yes = next_token_logits[:, yes_token_id].float()
                logit_no = next_token_logits[:, no_token_id].float()
                p_no_binary = torch.sigmoid(logit_no - logit_yes)
                protect_no = p_no_binary >= gate_threshold

                cd_logits = torch.where(
                    protect_no.unsqueeze(-1),
                    next_token_logits,
                    cd_logits,
                )

                # 公式runnerはbatch size 1なので、各質問のGate情報を
                # 出力JSONLへ保存できるようmodel属性へ記録
                self.precision_gate_triggered = bool(protect_no[0].item())
                self.precision_gate_p_no = float(p_no_binary[0].item())
